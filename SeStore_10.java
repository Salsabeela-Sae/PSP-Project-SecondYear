...
public class SeStore_10 {
    ...
    private static void activeUser(String[] activeUserData) throws IOException {
        ...
        while (true) {
            if (activeUserData[4].charAt(6) == '0') {
                ...
                System.out.println("Email: " + activeUserData[3].substring(0, 2) + "**@" + activeUserData[3].charAt(activeUserData[3].indexOf("@") + 1) + 
                activeUserData[3].charAt(activeUserData[3].indexOf("@") + 2) + "**");
            ...
            if (checkInfo == true) {
                ...
                try (Scanner readFile = new Scanner(new File("/Users/salsabeelasa-e/Documents/Year2_Semester1/PSP/src/Member.txt"))) {
                    ...
                    if (Integer.parseInt(arr_data[0]) == newID) {
                        ...
                        String dataPass = "" + arr_data[4].charAt(9) + arr_data[4].charAt(10) + arr_data[4].charAt(13) + arr_data[4].charAt(14) + 
                        arr_data[4].charAt(15) + arr_data[4].charAt(16);
    ...
    // Method สำหรับการสั่งซื้อจากรถเข็น
    public static void placeOrder(List<String[]> cart, String idMember) {
        // ตรวจสอบว่ารถเข็นว่างหรือไม่
        if (cart.isEmpty()) {
            System.out.println("Your cart is empty. Please add products to your cart before placing an order.");
            return; // ออกจาก method หากรถเข็นว่าง
        }

        // ดึงวันที่ปัจจุบัน
        String orderDate = new SimpleDateFormat("yyyy-MM-dd").format(new Date());

        // วนลูปผ่านรายการในรถเข็นและประมวลผลการสั่งซื้อ
        for (String[] cartItem : cart) {
            String productId = cartItem[1]; // รหัสสินค้า
            int quantity = Integer.parseInt(cartItem[2]); // จำนวนสินค้า

            // ที่นี่คุณสามารถเพิ่มตรรกะในการจัดการการสั่งซื้อได้
            // เช่น ลดสต็อก สร้างรายละเอียดการสั่งซื้อในฐานข้อมูล ฯลฯ
            System.out.println("Order placed for Product ID: " + productId + " with Quantity: " + quantity + " on " + orderDate);
        }

        // หลังจากประมวลผลการสั่งซื้อแล้ว ให้เคลียร์รถเข็น
        cart.clear();
        System.out.println("Your order has been placed successfully on " + orderDate + ". Your cart has been cleared.");
    }
}
